...
describe('validations', () => {
  test('valid', () => {
    expect(() => { map(['cool']) }).not.toThrow()
    expect(() => { map(123) }).not.toThrow()
    expect(() => { map({ foo: 'bar' }) }).not.toThrow()
    expect(() => { map('neat') }).not.toThrow()
    expect(() => { map({ '@path': '$.foo.bar' }) }).not.toThrow()
    expect(() => { map({ a: 1, b: { '@path': '$.foo.bar' } }) }).not.toThrow()
  })

  test('invalid', () => {
    expect(() => { map({ a: 1, '@field': '$.foo.bar' }) }).toThrow()
    expect(() => { map({ oops: { '@merge': [{}, 123] } }) }).toThrow()
  })
})
...
describe('remove undefined values in objects', () => {
  ...
  test('nested', () => {
    expect(
      map({ x: { y: undefined, z: 1 }, foo: 1 }, {})
    ).toEqual({ x: { z: 1 }, foo: 1 })
    expect(
      map({ x: { y: { z: undefined } }, foo: 1 }, {})
    ).toEqual({ x: { y: {} }, foo: 1 })
  })
})
