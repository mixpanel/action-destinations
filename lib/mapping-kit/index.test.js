const { map, validate } = require('./index')

describe('validate()', () => {
  test('valid', () => {
    expect(() => { validate(['cool']) }).not.toThrow()
    expect(() => { validate(123) }).not.toThrow()
    expect(() => { validate({ foo: 'bar' }) }).not.toThrow()
    expect(() => { validate('neat') }).not.toThrow()
    expect(() => { validate({ '@path': '$.foo.bar' }) }).not.toThrow()
    expect(() => { validate({ a: 1, b: { '@path': '$.foo.bar' } }) }).not.toThrow()
  })

  test('invalid', () => {
    expect(() => { validate({ a: 1, '@field': '$.foo.bar' }) }).toThrow()
    expect(() => { validate({ oops: { '@merge': [{}, 123] } }) }).toThrow()
  })
})
