...
registerDirective('@base64', (string, payload) => {
  const s = resolve(string, payload)
  const stringType = realTypeOf(s)
  if (stringType !== 'string') throw new Error(`@base64: expected string, got ${stringType}`)

  return Buffer.from(s).toString('base64')
})

registerDirective('@field', (field, payload) => {
  const f = resolve(field, payload)
  const fieldType = realTypeOf(f)
  if (fieldType !== 'string') throw new Error(`@field: expected field string, got ${fieldType}`)

  return safeGet(payload, f)
})
