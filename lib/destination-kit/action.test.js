...
describe('validateSettings', () => {
  ...
  test('invalid', async () => {
    ...
    expect(result.error.toString()).toBe('Error: Foo should be a number but it was a string.')
  })
})
...
describe('request()', () => {
  test('200 request', async () => {
    ...
    const out = await action().request((req) => (req.get(server.url))).execute({})
    server.close()
