...
class Entrypoint extends Step {
  constructor (settingsSchema, payloadSchema) {
    super()
    this.steps = new Steps()
    this.steps.push(new MapAndValidateInput(settingsSchema, payloadSchema))
  }
...
class MapAndValidateInput extends Step {
  constructor (settingsSchema = null, payloadSchema = null) {
    super()
    ...
    if (settingsSchema) this.validateSettings = ajv.compile(settingsSchema)
    else this.validateSettings = () => true

    if (payloadSchema) this.validatePayload = ajv.compile(payloadSchema)
    else this.validatePayload = () => true
  }

  _execute (ctx) {
    if (ctx.settings) ctx.settings = map(ctx.settings, ctx.payload)
    if (!this.validateSettings(ctx.settings)) {
      throw new Error(`Settings are invalid: ${this._ajvErrorMessage(this.validateSettings.errors)}`)
    }

    if (ctx.mapping) ctx.payload = map(ctx.mapping, ctx.payload)
    if (!this.validatePayload(ctx.payload)) {
      throw new Error(`Payload is invalid: ${this._ajvErrorMessage(this.validatePayload.errors)}`)
    }
  }
...
class FanOut extends Step {
  ...
  _on (on, ctx) {
    ...
    const found = JSONPath({ path: on, json: ctx.payload })
