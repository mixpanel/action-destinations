...
describe('action()', () => {
  ...
  describe('settings schema', () => {
    ...
    test('valid', async () => {
      ...
      const payload = { foo: 1 }
      let ctx = {}
      const result = await action(schema).do((c) => { ctx = c }).execute({ settings, payload })
      expect(result.error).toBeNull()
    ...
    test('invalid', async () => {
      ...
      const result = await action(schema).execute({ settings, payload })
  ...
  describe('payload schema', () => {
    ...
    test('valid', async () => {
      ...
      const payload = { foo: 1 }
      let ctx = {}
      const result = await action(null, schema).do((c) => { ctx = c }).execute({ mapping, payload })
      expect(result.error).toBeNull()
      expect(ctx.payload).toStrictEqual({ a: 1 })
    })
    ...
    test('invalid', async () => {
      const mapping = { a: { '@path': '$.foo' } }
      const payload = { foo: 'wrong type' }
      const result = await action(null, schema).execute({ mapping, payload })
      expect(result.error).toBeInstanceOf(Error)
