...
export function formatPhone(phone?: String): String {
  if (!phone) return ''
  return createHash('sha256').update(`+${phone.replace(/[^0-9]/g, '')}`).digest('base64')
}

export function formatFirstName(firstName?: String): String {
  if (!firstName) return ''
  return createHash('sha256').update(firstName.toLowerCase().replace(/[^a-z]/g, '')).digest('base64')
}

export function formatLastName(lastName?: String): String {
  if (!lastName) return ''
  return createHash('sha256').update(lastName.toLowerCase().replace(/[^a-z]/g, '')).digest('base64')
}
