...
class Step {
  ...
  async execute(ctx) {
    ...
    if (result.error) {
      this._log(
        `Failed after ${durationMs(result.startedAt, result.finishedAt)}: ${
...
class FanOut extends Step {
  ...
  async _execute(ctx) {
    ...
    return await Promise.all(
      values.map(val =>
        this._executeForValue.bind(this)(ctx, this.opts.as, val)
      )
    )
  }
  ...
  cachedRequest(config) {
    const step = new CachedRequest(this._baseRequest, config)
...
class CachedRequest extends Request {
  constructor(extensions, config) {
    ...
    if (typeof ttl !== 'number')
      throw new Error('"ttl" must be a number (seconds)')
