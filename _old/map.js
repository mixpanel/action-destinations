...
function T(fn) {
    ...
    base.event.stmt = function(strs, values) {
        return () => {
            let ctxConsts = Object.entries(base.event.event).map(([k,v])=>{
                if (k === 'stmt') return ''
...
function stmt(strs, values) {
    if (values) throw new Error('not implemented')
    return (ctx) => {
        let ctxConsts = Object.entries(ctx).map(([k,v])=>{
            if (typeof v === 'function') v = v()
            return `const ${k} = ${JSON.stringify(v)}`
        }).join(';')
...
const map = {
    if: (statement, value)  => {
        ...
        if (Boolean(statement({value: value}))) {
...
T(({event, helpers}) => ({
    ...
    last_request_at: map.if(
        event.stmt`options.active === true`,
        transform.unixTime(event`.timestamp`)
    ),

    company: map.if(
        stmt`value && value.length > 0`,
        helpers.companyName(),
    ),
